...
fn new_sets_https_base_url() {
    let client = ApiClient::new(27124, "test-key".to_string()).unwrap();
    assert!(
...
fn new_embeds_port_in_base_url() {
    let client = ApiClient::new(12345, "key".to_string()).unwrap();
    assert_eq!(client.base_url(), "https://127.0.0.1:12345");
}

#[test]
fn new_uses_default_obsidian_port() {
    let client = ApiClient::new(27124, "key".to_string()).unwrap();
...
fn new_with_different_ports() {
    for port in [80, 443, 8080, 27124, 65535] {
        let client = ApiClient::new(port, "k".to_string()).unwrap();
...
async fn check_connection_returns_false_when_no_server() {
    ...
    let client = ApiClient::new(19999, "fake-key".to_string()).unwrap();
